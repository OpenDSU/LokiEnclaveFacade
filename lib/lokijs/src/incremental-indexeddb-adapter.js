(function (root, factory) {
    ...
    return (function () {
        ...
        /* jshint -W030 */
        let DEBUG = typeof window !== 'undefined' && !!window.__loki_incremental_idb_debug;
        ...
        IncrementalIndexedDBAdapter.prototype._getChunk = function (collection, chunkId) {
            // 0-99, 100-199, etc.
            let minId = chunkId * this.chunkSize;
            let maxId = minId + this.chunkSize - 1;

            // use idIndex to find first collection.data position within the $loki range
            collection.ensureId();
            let idIndex = collection.idIndex;
            ...
            // verify
            let firstElement = collection.data[firstDataPosition];
            if (!(firstElement && firstElement.$loki >= minId && firstElement.$loki <= maxId)) {
                throw new Error("broken inletiant firstelement");
            }

            let lastElement = collection.data[lastDataPosition];
            if (!(lastElement && lastElement.$loki >= minId && lastElement.$loki <= maxId)) {
                throw new Error("broken inletiant lastElement");
            }

            // this will have *up to* 'this.chunkSize' elements (might have less, because $loki ids
            // will have holes when data is deleted)
            let chunkData = collection.data.slice(firstDataPosition, lastDataPosition + 1);

            if (chunkData.length > this.chunkSize) {
                throw new Error("broken inletiant - chunk size");
            }
         ...
         *
         * @example
         * let idbAdapter = new IncrementalIndexedDBAdapter();
        ...
        IncrementalIndexedDBAdapter.prototype.saveDatabase = function (dbname, getLokiCopy, callback) {
            ...
            try {
                ...
                let tx = this.idb.transaction(['LokiIncrementalData'], "readwrite");
                ...
                let store = tx.objectStore('LokiIncrementalData');

                let performSave = function (maxChunkIds) {
                    try {
                        let incremental = !maxChunkIds;
                        let chunkInfo = that._putInChunks(store, getLokiCopy(), incremental, maxChunkIds);
                ...
                let getAllKeysThenSave = function () {
                    ...
                    idbReq(store.getAllKeys(), function (e) {
                        let maxChunkIds = getMaxChunkIds(e.target.result);
        ...
        function getMaxChunkIds(allKeys) {
            ...
            allKeys.forEach(function (key) {
                ...
                if (keySegments.length === 3 && keySegments[1] === "chunk") {
                    let collection = keySegments[0];
                    let chunkId = parseInt(keySegments[2]) || 0;
                    let currentMax = maxChunkIds[collection];
        ...
        function lokiChunkVersionId(chunk) {
            try {
                if (chunk) {
                    let loki = JSON.parse(chunk.value);
        ...
        IncrementalIndexedDBAdapter.prototype._putInChunks = function (idbStore, loki, incremental, maxChunkIds) {
            ...
            let prepareCollection = function (collection, i) {
                ...
                if (incremental) {
                    ...
                    // add all chunks
                    let maxChunkId = (collection.maxId / that.chunkSize) | 0;
                    ...
                    let persistedMaxChunkId = maxChunkIds[collection.name] || 0;
                    for (let k = maxChunkId + 1; k <= persistedMaxChunkId; k += 1) {
                        let deletedChunkName = collection.name + ".chunk." + k;
                ...
                if (collection.dirty || dirtyChunks.size || !incremental) {
                    ...
                    let metadataChunk = JSON.stringify(collection);
            ...
            let serializedMetadata = JSON.stringify(loki);
        ...
        IncrementalIndexedDBAdapter.prototype._getAllChunks = function (dbname, callback) {
            ...
            let tx = this.idb.transaction(['LokiIncrementalData'], "readonly");
            let store = tx.objectStore('LokiIncrementalData');

            let deserializeChunk = this.options.deserializeChunk;
            ...
            function getMegachunks(keys) {
                let megachunkCount = that.megachunkCount;
                let keyRanges = createKeyRanges(keys, megachunkCount);

                let allChunks = [];
                let megachunksReceived = 0;

                function processMegachunk(e) {
                    // let debugMsg = 'processing chunk ' + megachunkIndex + ' (' + keyRange.lower + ' -- ' + keyRange.upper + ')'
                    // DEBUG && console.time(debugMsg);
                    let megachunk = e.target.result;
        ...
        function createKeyRanges(keys, count) {
            let countPerRange = Math.floor(keys.length / count);
            let keyRanges = [];
            let minKey, maxKey;
