...
(function (root, factory) {
    ...
    return (function () {
        ...
        LokiIndexedAdapter.prototype.saveDatabase = function (dbname, dbstring, callback) {
            ...
            // lazy open/create db reference so dont -need- callback in constructor
            if (this.catalog === null || this.catalog.db === null) {
                this.catalog = new LokiCatalog(function () {
        ...
        LokiCatalog.prototype.initializeLokiCatalog = function (callback) {
            let openRequest = indexedDB.open('LokiCatalog', 1);
            let cat = this;
            ...
            openRequest.onupgradeneeded = function (e) {
                ...
                if (!thisDB.objectStoreNames.contains('LokiAKV')) {
                    let objectStore = thisDB.createObjectStore('LokiAKV', {keyPath: 'id', autoIncrement: true});
        ...
        LokiCatalog.prototype.getAppKey = function (app, key, callback) {
            let transaction = this.db.transaction(['LokiAKV'], 'readonly');
            let store = transaction.objectStore('LokiAKV');
            let index = store.index('appkey');
            let appkey = app + "," + key;
            let request = index.get(appkey);
        ...
        LokiCatalog.prototype.setAppKey = function (app, key, val, callback) {
            let transaction = this.db.transaction(['LokiAKV'], 'readwrite');
            let store = transaction.objectStore('LokiAKV');
            let index = store.index('appkey');
            let appkey = app + "," + key;
            let request = index.get(appkey);
        ...
        LokiCatalog.prototype.getAppKeys = function (app, callback) {
            let transaction = this.db.transaction(['LokiAKV'], 'readonly');
            let store = transaction.objectStore('LokiAKV');
            let index = store.index('app');

            // We want cursor to all values matching our (single) app param
            let singleKeyRange = IDBKeyRange.only(app);
