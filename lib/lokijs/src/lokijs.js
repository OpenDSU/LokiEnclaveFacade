...
(function (root, factory) {
  ...
  return (function () {
    ...
    function LokiPartitioningAdapter(adapter, options) {
      adapter = adapter || new LokiFsAdapter();
      this.mode = "reference";
      this.adapter = null;
      this.options = options || {};
      this.dbref = null;
    ...
    LokiFsAdapter.prototype.saveDatabase = function saveDatabase(dbname, dbstring, callback) {
      ...
      var dirname = require("path").dirname(dbname);

      function writeFileAndRename(tmpdbname, dbname, dbstring, callback) {
        self.fs.writeFile(tmpdbname, dbstring, function writeFileCallback(err) {
          if (err) {
            callback(err);
          } else {
      ...
      this.fs.access(dirname, function checkAccessCallback(err) {
        if (err) {
          self.fs.mkdir(dirname, { recursive: true }, function mkdirCallback(err) {
            if (err) {
              callback(err);
            } else {
              writeFileAndRename(tmpdbname, dbname, dbstring, callback);
            }
          });
        } else {
          writeFileAndRename(tmpdbname, dbname, dbstring, callback);
    ...
    Loki.prototype.saveDatabase = function (callback) {
      // Start measuring time using Date.now() for higher precision
