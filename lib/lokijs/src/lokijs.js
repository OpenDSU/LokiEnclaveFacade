...
(function (root, factory) {
    ...
    return (function () {
        ...
        let Comparators = {
            aeq: aeqHelper, lt: ltHelper, gt: gtHelper
        };
        ...
        let valueLevelOps = ['$eq', '$aeq', '$ne', '$dteq', '$gt', '$gte', '$lt', '$lte', '$jgt', '$jgte', '$jlt', '$jlte', '$type'];
        ...
        function clone(data, method) {
            ...
            let cloneMethod = method || 'parse-stringify', cloned;
        ...
        function Loki(filename, options) {
            ...
            this.events = {
                'init': [], 'loaded': [], 'flushChanges': [], 'close': [], 'changes': [], 'warning': []
            };
        ...
        Loki.prototype.configureOptions = function (options, initialConfig) {
            let defaultPersistence = {
                'NODEJS': 'fs', 'BROWSER': 'localStorage', 'CORDOVA': 'localStorage', 'MEMORY': 'memory'
            }, persistenceMethods = {
                'fs': LokiFsAdapter, 'localStorage': LokiLocalStorageAdapter, 'memory': LokiMemoryAdapter
            };
        ...
        Loki.prototype.serializeDestructured = function (options) {
            ...
            if (options.partitioned === true && options.hasOwnProperty("partition") && options.partition >= 0) {
                return this.serializeCollection({
                    delimited: options.delimited, delimiter: options.delimiter, collectionIndex: options.partition
                });
            }
            ...
            for (idx = 0; idx < this.collections.length; idx++) {
                result = this.serializeCollection({
                    delimited: options.delimited, delimiter: options.delimiter, collectionIndex: idx
                });
        ...
        Loki.prototype.loadJSONObject = function (dbObject, options) {
            let i = 0, len = dbObject.collections ? dbObject.collections.length : 0, coll, copyColl, clen, j, loader,
                collObj;
    ...
    -------------------*/
            ...
            if (this.options.asyncResponses) {
                setTimeout(function () {
                    ...
                    self.hashStore[dbname] = {
                        savecount: saveCount + 1, lastsave: new Date(), value: dbstring
                    };
        ...
        LokiPartitioningAdapter.prototype.loadNextPartition = function (partition, callback) {
            ...
            this.adapter.loadDatabase(keyname, function (result) {
                let data = self.dbref.deserializeCollection(result, {delimited: true, collectionIndex: partition});
        ...
        LokiPartitioningAdapter.prototype.loadNextPage = function (callback) {
            ...
            let keyname = this.dbname + "." + this.pageIterator.collection + "." + this.pageIterator.pageIndex;
            let self = this;
        ...
        LokiPartitioningAdapter.prototype.saveNextPartition = function (callback) {
            ...
            if (this.options.paging && partition !== -1) {
                this.pageIterator = {
                    collection: partition, docIndex: 0, pageIndex: 0
                };
            ...
            let result = this.dbref.serializeDestructured({
                partitioned: true, delimited: true, partition: partition
            });
        ...
        LokiPartitioningAdapter.prototype.saveNextPage = function (callback) {
            ...
            let pageSaveCallback = function (err) {
                pageBuilder = "";
        ...
        Loki.prototype.loadDatabaseInternal = function (options, callback) {
            let cFun = callback || function (err, data) {
                if (err) {
                    throw err;
                }
            }, self = this;
        ...
        Loki.prototype.saveDatabaseInternal = function (callback) {
            let cFun = callback || function (err) {
                if (err) {
                    throw err;
                }
            ...
            if (this.persistenceAdapter.mode === "incremental") {
                ...
                this.persistenceAdapter.saveDatabase(this.filename, function getLokiCopy() {
                    ...
                    let lokiCopy = self.copy({removeNonSerializable: true});
        ...
        Resultset.prototype.simplesort = function (propname, options) {
            let eff, targetEff = 10, dc = this.collection.data.length, frl = this.filteredrows.length,
            ...
            let wrappedComparer = (function (prop, desc, data) {
                let val1, val2, arr;
                return function (a, b) {
                    if (~prop.indexOf('.')) {
                        arr = prop.split('.');
        ...
        Resultset.prototype.compoundsort = function (properties) {
            ...
            let wrappedComparer = (function (props, data) {
                return function (a, b) {
        ...
        Resultset.prototype.findOr = function (expressionArray) {
            let fr = null, fri = 0, frlen = 0, docset = [], idxset = [], idx = 0;
        ...
        Resultset.prototype.find = function (query, firstOnly) {
            ...
            let queryObject = query || 'getAll', p, property, queryObjectOp, obj, operator, value, key,
                searchByIndex = false, result = [], filters = [], index = null;
            ...
            let usingDotNotation = (property.indexOf('.') !== -1);

            // if an index exists for the property being queried against, use it
            // for now only enabling where it is the first filter applied and prop is indexed
            let doIndexCheck = !this.filterInitialized;
            ...
            // first chained query so work against data[] but put results in filteredrows
                ...
                if (!searchByIndex) {
                    ...
                    // search by index
                    let segm = this.collection.calculateRange(operator, property, value);
        ...
        Resultset.prototype.eqJoin = function (joinData, leftJoinKey, rightJoinKey, mapFun, dataOptions) {
            ...
            let leftData = [], leftDataLength, rightData = [], rightDataLength, key, result = [],
                leftKeyisFunction = typeof leftJoinKey === 'function',
                rightKeyisFunction = typeof rightJoinKey === 'function', joinMap = {};
        ...
        DynamicView.prototype.rematerialize = function (options) {
            ...
            let wasFrozen = Object.isFrozen(this.filterPipeline);
        ...
        DynamicView.prototype.toJSON = function () {
            let copy = new DynamicView(this.collection, this.name, this.options);
        ...
        DynamicView.prototype.removeFilters = function (options) {
            ...
            let wasFrozen = Object.isFrozen(this.filterPipeline);
            let filterChanged = this.filterPipeline.length > 0;
            // keep ordered filter pipeline
            this.filterPipeline = [];
            if (wasFrozen) {
                Object.freeze(this.filterPipeline);
        ...
        DynamicView.prototype.reapplyFilters = function () {
            ...
            let filters = this.filterPipeline;
            let wasFrozen = Object.isFrozen(filters);
            this.filterPipeline = [];
        ...
        DynamicView.prototype.applyFilter = function (filter) {
            let idx = this._indexOfFilterWithId(filter.uid);
            if (idx >= 0) {
                let wasFrozen = Object.isFrozen(this.filterPipeline);
                if (wasFrozen) {
                    this.filterPipeline = this.filterPipeline.slice();
        ...
        DynamicView.prototype.removeFilter = function (uid) {
            let idx = this._indexOfFilterWithId(uid);
            if (idx < 0) {
        ...
        DynamicView.prototype.evaluateDocument = function (objIndex, isNew) {
            ...
            let ofr = this.resultset.filteredrows;
            let oldPos = (isNew) ? (-1) : (ofr.indexOf(+objIndex));
            let oldlen = ofr.length;

            // creating a 1-element resultset to run filter chain ops on to see if that doc passes filters;
            // mostly efficient algorithm, slight stack overhead price (this function is called on inserts and updates)
            let evalResultset = new Resultset(this.collection);
            ...
            let newPos = (evalResultset.filteredrows.length === 0) ? -1 : 0;
        ...
        DynamicView.prototype.removeDocument = function (objIndex) {
            ...
            let adjels = [];
            let drs = this.resultset;
            let fr = this.resultset.filteredrows;
            let frlen = fr.length;
        ...
        function Collection(name, options) {
            ...
            function getObjectDelta(oldObject, newObject) {
                let propertyNames = newObject !== null && typeof newObject === 'object' ? Object.keys(newObject) : null;
                    ...
                    for (let i = 0; i < propertyNames.length; i++) {
                        ...
                        if (newObject.hasOwnProperty(propertyName)) {
                            if (!oldObject.hasOwnProperty(propertyName) || self.uniqueNames.indexOf(propertyName) >= 0 || propertyName === '$loki' || propertyName === 'meta') {
                                ...
                                let propertyDelta = getObjectDelta(oldObject[propertyName], newObject[propertyName]);
    ...
    +----------------------------*/
        ...
        Collection.prototype.ensureIndex = function (property, force) {
            ...
            let index = {
                'name': property, 'dirty': true, 'values': this.prepareFullDocIndex()
            };
            this.binaryIndices[property] = index;

            let wrappedComparer = (function (prop, data) {
                let val1, val2;
        ...
        Collection.prototype.insert = function (doc, overrideAdaptiveIndices) {
            ...
            let adaptiveBatchOverride = overrideAdaptiveIndices && !this.cloneObjects && this.adaptiveBinaryIndices && Object.keys(this.binaryIndices).length > 0;
        ...
        Collection.prototype.clear = function (options) {
            ...
            // clear indices but leave definitions in place
                ...
                let keys = Object.keys(this.binaryIndices);
       ...
       */
            try {
                ...
                // now that we can efficiently determine the data[] position of newly added document,
                // submit it for all registered DynamicViews to evaluate for inclusion/exclusion
                let dvlen = this.DynamicViews.length;
        ...
        Collection.prototype.removeBatchByPositions = function (positions) {
            ...
            let xo = {};
            let dlen, didx, idx;
            let bic = Object.keys(this.binaryIndices).length;
            let uic = Object.keys(this.constraints.unique).length;
            let adaptiveOverride = this.adaptiveBinaryIndices && Object.keys(this.binaryIndices).length > 0;
            let doc, self = this;
            ...
            try {
                ...
                if ((dlen > 0) || (bic > 0) || (uic > 0)) {
                    ...
                    if (uic) {
                        this.uniqueNames.forEach(function (key) {
                            let index = self.getUniqueIndex(key);
                            if (index) {
                                for (idx = 0; idx < len; idx++) {
        ...
        Collection.prototype.remove = function (doc) {
            ...
            try {
                this.startTransaction();
                let arr = this.get(doc.$loki, true), // obj = arr[0],
                    position = arr[1];
                let self = this;
                this.uniqueNames.forEach(function (key) {
    ...
    +----------------------*/
        ...
        Collection.prototype.get = function (id, returnPosition) {
            ...
            let retpos = returnPosition || false, data = this.idIndex, max = data.length - 1, min = 0, mid;
        ...
        Collection.prototype.getBinaryIndexPosition = function (dataPosition, binaryIndexName) {
            let val = Utils.getIn(this.data[dataPosition], binaryIndexName, true);
        ...
        Collection.prototype.adaptiveBinaryIndexInsert = function (dataPosition, binaryIndexName) {
            let usingDotNotation = (binaryIndexName.indexOf('.') !== -1);
            let index = this.binaryIndices[binaryIndexName].values;
            ...
            let idxPos = (index.length === 0) ? 0 : this.calculateRangeStart(binaryIndexName, val, true, usingDotNotation);
        ...
        Collection.prototype.adaptiveBinaryIndexRemove = function (dataPosition, binaryIndexName, removedFromIndexOnly) {
            let bi = this.binaryIndices[binaryIndexName];
            ...
            if (Array.isArray(dataPosition)) {
                ...
                // we were passed an array (batch) of documents so use this 'batch optimized' algorithm
                    ...
                    let sortedPositions = dataPosition.slice();
        ...
        Collection.prototype.calculateRange = function (op, prop, val) {
            ...
            let usingDotNotation = (prop.indexOf('.') !== -1);

            let minVal = Utils.getIn(rcd[index[min]], prop, usingDotNotation);
            let maxVal = Utils.getIn(rcd[index[max]], prop, usingDotNotation);
        ...
        Collection.prototype.by = function (field, value) {
            ...
            let result = this.getUniqueIndex(field, true).get(value);
            if (!this.cloneObjects) {
                return result;
            } else {
        ...
        Collection.prototype.findOne = function (query) {
            ...
            let result = this.chain().find(query, true).data();
        ...
        Collection.prototype.commitStage = function (stageName, message) {
            ...
            for (prop in stage) {
                ...
                this.commitLog.push({
                    timestamp: timestamp, message: message, data: JSON.parse(JSON.stringify(stage[prop]))
                });
            }
        ...
        Collection.prototype.mode = function (field) {
            let dict = {}, data = this.extract(field);
        ...
        SortedIndex.prototype = {
            keys: [], values: [], // set the default sort
            sort: function (a, b) {
                return (a < b) ? -1 : ((a > b) ? 1 : 0);
            }, bs: function () {
                return new BSonSort(this.sort);
            ...
            set: function (key, value) {
                let pos = binarySearch(this.keys, key, this.sort);
                if (pos.found) {
            ...
            get: function (key) {
                let bsr = binarySearch(this.keys, key, this.sort);
                if (bsr.found) {
            ...
            remove: function (key, value) {
                let pos = binarySearch(this.keys, key, this.sort).index;
        ...
        Loki.persistenceAdapters = {
            fs: LokiFsAdapter, localStorage: LokiLocalStorageAdapter
        };
        Loki.aeq = aeqHelper;
        Loki.lt = ltHelper;
        Loki.gt = gtHelper;
