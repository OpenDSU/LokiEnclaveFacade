...
let isError = require('util').isError;
...
function encrypt(input, secret) {
    ...
    let salt = cryptoLib.randomBytes(KEY_LENGTH / 8),
        iv = cryptoLib.randomBytes(16);

    try {

        let key = cryptoLib.pbkdf2Sync(secret, salt, ITERATIONS, KEY_LENGTH / 8, 'sha1'),
...
function decrypt(input, secret) {
    ...
    let salt = new Buffer(input.salt, 'base64'),
    ...
    try {
        ...
        let key = cryptoLib.pbkdf2Sync(secret, salt, iterations, keyLength / 8, 'sha1'),
...
lokiCryptedFileAdapter.prototype.loadDatabase = function loadDatabase(dbname, callback) {
    ...
    let cFun = callback || console.log;

    fs.readFile(dbname, 'utf8', function (err, data) {
        let decrypted = err || decrypt(data, secret);
        cFun(decrypted);
    });
};
...
lokiCryptedFileAdapter.prototype.saveDatabase = function saveDatabase(dbname, dbstring, callback) {
    let cFun = callback || function () {
    };
    let encrypted = encrypt(dbstring, this.secret);
